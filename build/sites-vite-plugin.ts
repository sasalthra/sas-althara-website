...
export function sites(): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.config.logger.info('Local simulated sign-in disabled for this CRM.');
      server.middlewares.use((request, response, next) => {
        ...
        if (signIn) { response.statusCode=403; response.setHeader('Content-Type','text/plain; charset=utf-8'); response.end('الدخول التجريبي المحلي معطّل. استخدم رابط المعاينة الخاص لتسجيل الدخول الحقيقي.'); return; }
        if (!signIn && !signOut) {
          if (false && signInCookies.length === 1 && signInCookies[0] === "1") {
