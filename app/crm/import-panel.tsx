...
export default function ImportPanel({onSaved}:{onSaved:()=>void}){
 ...
 const [assignMode,setAssignMode]=useState<Assignment['mode']>('unassigned');
 ...
 async function run(mode:'preview'|'commit'){
  ...
  try{
   if(mode==='commit'){
    if(assignMode==='one'&&selected.length!==1)throw Error('اختر مندوباً واحداً أو أبقِ الاستيراد بدون تعيين');
 ...
 function toggleRep(id:string){
  setSelected(current=>{
   if(assignMode==='one')return current[0]===id?[]:[id];
 ...
 return <section dir="rtl" className="panel space-y-4">
  ...
  <p>أرقام الجوال السعودية تُوحّد للمقارنة. التكرار بحسب الجوال؛ لا يستبدل أي عميل موجود. إن لم يُربط عمود العقار يُحفظ «غير محدد» حتى يظهر العميل في السجل. المرحلة تُطابق مراحل النظام فقط. الأعمدة غير المربوطة تُحفظ مع الصف الأصلي. الصفوف غير الصالحة لا تُحذف من الملف. صيغ Excel لا تُحسب؛ استخدم قيماً محفوظة.</p>
  ...
  {preview.length>0&&<>
   ...
   <div className="import-assign">
    <h3>تعيين العملاء بعد المراجعة</h3>
    <p className="subtle">بدون تعيين لا يظهر العميل لمندوب المبيعات في سجل العملاء والمتابعات. عيّن الكل لمندوب واحد أو وزّعهم بالتساوي (دوري) على المحددين.</p>
    ...
    <div className="import-assign-options">
     ...
     <button type="button" className="import-assign-option" data-active={assignMode==='one'} aria-pressed={assignMode==='one'} onClick={()=>{setAssignMode('one');setSelected(current=>current.slice(0,1));}}>
      <UserPlus className="import-assign-icon" aria-hidden="true" size={20}/>
      <span><strong>تعيين الكل لمندوب واحد</strong><small>كل الصفوف الجاهزة تُسند لنفس مندوب المبيعات</small></span>
     </button>
    ...
    {assignMode!=='unassigned'&&(
     <div className="import-assign-reps">
      <p>{assignMode==='one'?'اختر مندوباً واحداً':'حدد مندوبين أو أكثر'}</p>
      {sales.length===0&&!usersError&&<p className="subtle">لا يوجد مندوبو مبيعات نشطون في النظام.</p>}
      <ul>
       {sales.map(user=>(
        <li key={user.id}>
   ...
   <button className="primary" disabled={busy||!confirmed||readyCount===0} onClick={()=>void run('commit')}>تأكيد الاستيراد</button>
  </>}
 </section>;
}
