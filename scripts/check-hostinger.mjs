...
globalThis.qaSession={user:{email:profile.email},adminId:'google:test-admin',crmUserId:'google:test-admin',crmRole:'admin'};
globalThis.qaFailDB=false;
const salesId=crypto.randomUUID();
sqlite.prepare('INSERT INTO crm_users (id,name,username,role,active,email) VALUES (?,?,?,?,1,?)').run(salesId,'مندوب الاختبار','sales.rep','sales','sales@sas.test');
globalThis.sentMail=[];
const assignedLead={id:crypto.randomUUID(),name:'عميل معيّن',phone:'+966511111111',propertyId:property.id,stage:'new',notes:'يريد شقة',source:'manual',assignedTo:salesId};
assert.equal((await api.POST(make(assignedLead))).status,200);
assert.equal(globalThis.sentMail.length,2);
assert.equal(globalThis.sentMail[0].to,'sales@sas.test');
assert.match(globalThis.sentMail[0].subject,/عميل معيّن/);
assert.match(globalThis.sentMail[0].text,/يريد شقة/);
assert.match(globalThis.sentMail[0].html,/dir="rtl"/);
assert.equal(globalThis.sentMail[1].to.includes('sasalthra.sa@gmail.com'),true);
assert.match(globalThis.sentMail[1].text,/مندوب الاختبار/);
assert.match(globalThis.sentMail[1].text,/sales@sas.test/);
globalThis.sentMail=[];
assert.equal((await api.PATCH(make({...assignedLead,stage:'contacted',assignedTo:salesId}))).status,200);
assert.equal(globalThis.sentMail.length,0,'same assignee does not resend');
const salesTwo=crypto.randomUUID();
sqlite.prepare('INSERT INTO crm_users (id,name,username,role,active,email) VALUES (?,?,?,?,1,?)').run(salesTwo,'مندوب آخر','sales.two','sales','sales-two@sas.test');
assert.equal((await api.PATCH(make({...assignedLead,stage:'contacted',assignedTo:salesTwo}))).status,200);
assert.equal(globalThis.sentMail.length,2);
